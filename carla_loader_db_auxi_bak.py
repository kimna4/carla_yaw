...
def collate_data(batch):
    ...
    for sample in batch:
        ...
        target4.append(torch.Tensor(sample[4]))
        target5.append(torch.Tensor(sample[5]))
        target6.append(torch.Tensor(sample[6]))
        target7.append(torch.Tensor(sample[7]))
        target8.append(torch.Tensor(sample[8]))
        target9.append(torch.Tensor(sample[9]))

    return torch.stack(imgs, 0), torch.stack(target1, 0), torch.stack(target2, 0), torch.stack(target3, 0), \
        torch.stack(target4, 0), torch.stack(target5, 0), torch.stack(target6, 0), torch.stack(target7, 0), \
       torch.stack(target8, 0), torch.stack(target9, 0),
...
class CarlaH5Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(file_name, 'r') as h5_file:
            ...
            target_vec[command, :] = target[0, :3]
            # target_vec_tmp = target_vec.reshape(-1)
            mask_vec = np.zeros((4, 3), dtype=np.float32)
            mask_vec[command, :] = 1

            target_vec_lat = np.zeros((4), dtype=np.float32)
            target_vec_lat[command] = target[0, 0]
            mask_vec_lat = np.zeros((4), dtype=np.float32)
            mask_vec_lat[command] = 1

            target_vec_lon = np.zeros((4, 2), dtype=np.float32)
            target_vec_lon[command, :] = target[0, 1:3]
            mask_vec_lon = np.zeros((4, 2), dtype=np.float32)
            mask_vec_lon[command, :] = 1

            ''' relate yaw '''
            # pred_location_x, pred_location_y = get_predicted_wheel_location(0, 0, target[0, 0], 0, np.max(target[0, 10], 0) / 40, 0.1)
            pred_location_x, pred_location_y = get_predicted_wheel_location(0, 0, target[0, 0], 0,
                                                                            np.max(target[0, 10], 0) / 4, 0.1)

            print('x: {:.7f} , y: {:.7f}, steer: {:.5f}, speed: {:.5f} '.format(pred_location_x, pred_location_y,
                                                                                target[0, 0],
                                                                                np.max(target[0, 10], 0) / 4))


            target_vec_trans_x = np.zeros((4), dtype=np.float32)
            target_vec_trans_x[command] = pred_location_x
            target_vec_trans_y = np.zeros((4), dtype=np.float32)
            target_vec_trans_y[command] = pred_location_y

        return img, speed, target_vec.reshape(-1), mask_vec.reshape(-1), target_vec_lat.reshape(-1), \
               mask_vec_lat.reshape(-1), target_vec_lon.reshape(-1), mask_vec_lon.reshape(-1), \
               target_vec_trans_x.reshape(-1), target_vec_trans_y.reshape(-1)
